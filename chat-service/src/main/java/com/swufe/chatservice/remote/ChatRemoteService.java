...
public interface ChatRemoteService {
     ...
     * 远程调用获取手机号
     */
    @PostMapping("cgi-bin/message/subscribe/send?access_token={accessToken}")
    SubscribeMessageRespDTO sendSubscribeMessage(
            @PathVariable("accessToken") String accessToken,
            @RequestBody SubscribeMessageReqDTO subscribeMessageReqDTO
            );
