 ...
 * 乘车人持久层
 */
public interface LawyerMapper extends BaseMapper<LawyerDO> {
    // 使用 SQL 语句从数据库中随机抽取 5 个律师
    @Select("SELECT * FROM lawyer ORDER BY RAND() LIMIT 5")
    @Results({
            @Result(column = "domain", property = "domain", typeHandler = FastJsonListTypeHandler.class)
    })
    List<UserLawyerRespDTO> selectRandomLawyers();
}
